...
template <const int MOD> requires Prime<MOD> struct Modular {
    ...
    friend istream& operator>>(istream& in, Modular& n) {
        long long v; in >> v;
        n = Modular(v); return in;
    }
    friend ostream& operator<<(ostream& out, const Modular& n) { return out << n.value; }
    ...
    friend Modular pow(Modular a, integral auto p) {
    ...
    friend pair<Modular, Modular> divmod(const Modular& a, const Modular& b) { return make_pair(a.value / b.value, a.value % b.value); }

    template <typename T> static constexpr auto Integral = is_integral_v<T> || is_same_v<T, Modular>;
    template <typename T, typename U> requires Integral<T> && Integral<U> friend pair<Modular, Modular> divmod(T a, U b) { return divmod(Modular(a), Modular(b)); }
    template <typename T> requires Integral<T> friend Modular abs(T a) { return a < 0 ? -a : a; }
    template <typename T, typename U> requires Integral<T> && Integral<U> friend Modular gcd(T a, U b) { return b ? gcd(b, divmod(a, b).second) : a; }
    template <typename T, typename U> requires Integral<T> && Integral<U> friend Modular lcm(T a, U b) { return (a == MOD || b == MOD) ? 0 : (divmod(a, gcd(a, b)).first * b); }
};