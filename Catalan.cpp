...
template <const int MOD, const int N> requires Prime<MOD>
struct Combinatorics {
    using Mint = Modular<MOD>;
    std::vector<Mint> fact, invfact;
    constexpr Combinatorics() : fact(N), invfact(N) {
        fact[0] = 1;
        for (int i = 1; i < N; ++i) fact[i] = fact[i - 1] * i;
        invfact[N - 1] = inv(fact[N - 1]);
        for (int i = N - 2; i >= 0; --i) invfact[i] = invfact[i + 1] * (i + 1);
    }
    constexpr Mint C(int n, int k) const {
        if (k < 0 || k > n) return 0;
        return fact[n] * invfact[k] * invfact[n - k];
    }
    constexpr Mint P(int n, int k) const {
        if (k < 0 || k > n) return 0;
...
struct Catalan {
    using Mint = Modular<MOD>;
    Combinatorics<MOD, 2 * N> comb;
    std::vector<Mint> catalan;
    constexpr Catalan() : catalan(N) {
        for (int i = 0; i < N; ++i)
            catalan[i] = comb.C(2 * i, i) / (i + 1);
    }
    constexpr Mint operator[](int n) const { return catalan[n]; }
};